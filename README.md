...
1. Upload the files maintaining the structure above to the `main` branch of this repository.
...
1. **Access the Original Chat:** Click this link to open the resume builder conversation: [**Amon's Gemini Resume Chat**](https://gemini.google.com/share/94e35a470b76).
   *(Alternatively, you can open a new chat with Gemini at any time).*

2. **Upload the Latest Code:** To ensure Gemini has the most up-to-date version of your site, copy the code from `index.html` and paste it into the chat prompt.

3. **Give Clear Instructions:** Tell Gemini exactly what you want to change. Examples:
   * *"Add a new job under Work Experience: [Company Name], [Role], [Dates]. Here are my 3 bullet points:..."*
...
4. **Copy the Output:** Gemini will generate the newly updated `index.html` file. Simply copy that code, replace the code in your GitHub repository, and commit the changes! Your live website will update automatically.
